...
(function () {
  ...
  function detectSiteStructure() {
    ...
    showToast$3('🔍 正在尝试匹配相似规则...', 'info', 2000);
    ...
    if (bestMatch && bestMatchCount >= MIN_MATCH_CHAPTERS) {
      ...
      showToast$3(`✅ 自动匹配到相似规则：${bestMatch.name}`, 'success', 3000);
      return bestMatch;
    }

    console.warn('[站点检测] ❌ 智能回退失败，无法找到适用规则');
    showToast$3('⚠️ 未能识别当前站点，建议使用「规则管理」添加规则', 'warn', 4500);
